...
if [ ! -n "$ANDROID_UPDATE_FILTER"]; then
    export ANDROID_UPDATE_FILTER="${WERCKER_ANDROID_SDK_UPDATE_FILTER}"
else
    export ANDROID_UPDATE_FILTER="${ANDROID_UPDATE_FILTER}:,${WERCKER_ANDROID_SDK_UPDATE_FILTER}"
fi